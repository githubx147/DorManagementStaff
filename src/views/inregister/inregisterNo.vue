    ...
    <el-table :data="list" border style="width: 100%;font-size: 15px">
      ...
      <el-table-column label="操作"  align="center">
        <template slot-scope="scope">
          ...
          <el-button  v-if="scope.row.stuStatus=='离校'" type="primary" size="mini"  @click="enterSchool(scope.row.id)">入校操作</el-button>
...
export default {
  ...
  methods: {
    ...
    // 定义方法，请求接口调用
    enterSchool(id) {
      this.$confirm('点击确定修改为入校?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
         inregister.enterSchool(id)
          .then(result => {
            this.$message({
              type: 'success',
              message: '成功!'
    ...
    getList(page = 1) {
      this.current = page
      inregister.findLeaveSchool(this.current, this.limit, this.searchObj)
        .then((response) => {
